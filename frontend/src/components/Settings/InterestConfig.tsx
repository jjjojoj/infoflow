...
const MOCK_INTERESTS: Interest[] = [
  { id: 1, keyword: '昇腾910b', weight: 0.95, category: '核心技术', enabled: true },
  { id: 2, keyword: '鲲鹏', weight: 0.85, category: '平台环境', enabled: true },
  { id: 3, keyword: 'OCR', weight: 0.9, category: '核心技术', enabled: true },
  { id: 4, keyword: '信创', weight: 0.8, category: '平台环境', enabled: true },
  { id: 5, keyword: 'CANN', weight: 0.75, category: '工具框架', enabled: true },
  { id: 6, keyword: 'MindSpore', weight: 0.7, category: '工具框架', enabled: true },
  { id: 7, keyword: 'PaddleOCR', weight: 0.65, category: '工具框架', enabled: true },
  { id: 8, keyword: '文字识别', weight: 0.8, category: '核心技术', enabled: true },
  { id: 9, keyword: '模型部署', weight: 0.85, category: '学习成长', enabled: true },
  { id: 10, keyword: '国产化', weight: 0.7, category: '平台环境', enabled: false },
];

export default function InterestConfig() {
  const [interests, setInterests] = useState<Interest[]>(MOCK_INTERESTS);
  const [filter, setFilter] = useState('全部');
  const [description, setDescription] = useState('我是算法工程师，关注信创环境下的OCR模型部署，尤其是昇腾910b和鲲鹏平台上的推理优化与国产化适配方案。');
  ...
  const handleAdd = () => {
    ...
    const item: Interest = { id: Date.now(), keyword: newKeyword.trim(), weight: newWeight, category: newCategory, enabled: true };
  ...
  const toggleEnabled = (id: number) => {
    setInterests(prev => prev.map(i => i.id === id ? { ...i, enabled: !i.enabled } : i));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="mb-1.5 block text-sm text-[var(--color-text-secondary)]">兴趣描述（自然语言）</label>
        <textarea
          value={description}
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-[#374151] bg-[var(--color-bg-surface)] p-4">
        ...
        <button
          onClick={handleAdd}
          className="flex items-center gap-1 rounded-lg bg-[var(--color-accent)] px-3 py-1.5 text-sm font-medium text-white transition hover:bg-[var(--color-accent-light)]"
        >
          ...
          <div key={item.id} className={`rounded-xl border border-[#374151] bg-[var(--color-bg-surface)] p-3 transition ${!item.enabled ? 'opacity-50' : ''}`}>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded bg-[var(--color-accent-soft)] px-1.5 py-0.5 text-[10px] text-[var(--color-accent-light)]">{item.category}</span>
              </div>
